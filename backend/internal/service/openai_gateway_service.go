...
const (
	...
	// 该后缀是 UA 形态的唯一定义处；固定兜底 UA 的尾部版本与 codexCLIVersion 同源，
	// buildCodexCLIUserAgent 再通过版本重建器同步更新首段和尾段。
...
type OpenAIForwardResult struct {
	...
	// CodexTelemetry is bounded upstream evidence, independent of model/billing decisions.
...
type OpenAIGatewayService struct {
	codexDailySessionPool *CodexDailySessionPool
