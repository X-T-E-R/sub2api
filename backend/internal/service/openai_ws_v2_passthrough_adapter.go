...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	accountScopedFirst, accountScoped, scopeErr := projectCodexWSRequestBodyRaw(c, account, firstClientMessage)
	if scopeErr != nil {
	...
	firstClientMessage = s.applyModelReasoningFloor(account, capturedSessionModel, firstClientMessage, "reasoning.effort")
	...
	for {
		...
		cancelDial()
		if err != nil {
			telemetryAttempt.headers(handshakeHeaders, codextelemetry.HTTPHeaders, statusCode)
			telemetryAttempt.finish(nil, err)
		}
		if err == nil {
			break
		}
	...
	telemetryFrameConn := &codexTelemetryFrameConn{inner: upstreamFrameConn, c: c, account: account, headers: handshakeHeaders}
	relayUpstreamFrameConn := &openAIWSPassthroughFirstOutputFrameConn{
		inner:             telemetryFrameConn,
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			if isResponseCreate || eventType == "session.update" {
				accountScopedPayload, accountScoped, scopeErr := projectCodexWSRequestBodyRaw(c, account, payload)
				if scopeErr != nil {
			...
			if isResponseCreate {
				payload = s.applyModelReasoningFloor(account, model, payload, "reasoning.effort")
			}
		...
		Options: openaiwsv2.RelayOptions{
			...
			OnTurnComplete: func(turn openaiwsv2.RelayTurnResult) {
				...
				turnResult := &OpenAIForwardResult{
					...
					CodexTelemetry:                telemetryFrameConn.take(turn.RequestID),
