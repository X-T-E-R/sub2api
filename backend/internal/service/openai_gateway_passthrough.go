...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if account != nil && account.UsesOpenAICodexProtocol() {
		...
		accountScopedBody, accountScoped, scopeErr := projectCodexRequestBodyRaw(c, account, body)
		if scopeErr != nil {
			return nil, scopeErr
		}
		if accountScoped {
	...
	for {
		...
		telemetryAttempt := beginCodexHTTPAttempt(c, account)
	...
	forwardResult := &OpenAIForwardResult{
		...
		CodexTelemetry:                observedCodexTelemetry(c),
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	if account.UsesOpenAICodexProtocol() {
		...
		promptCacheKey := codexOriginalPromptCacheKey(c, account, strings.TrimSpace(gjson.GetBytes(body, "prompt_cache_key").String()))
	...
	applyCodexRequestHeaders(c, account, req.Header)
