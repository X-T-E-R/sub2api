...
func TestResolveGrokWSCacheIdentityUsesNormalizedEffectiveSeed(t *testing.T) {
	...
	effective, err := prepareOpenAIWSHTTPBridgeBody(account, seed)
	require.NoError(t, err)
	upstreamModel := resolveGrokWSUpstreamModel(account, seed, "grok")
	preCompatibility, _, _, err := patchGrokResponsesBodyWithClientToolsCompatibility(effective, upstreamModel, false)
	require.NoError(t, err)
	preCompatibilityIdentity := resolveGrokCacheIdentity(c, preCompatibility, "", upstreamModel)
	effective, _, _, err = patchGrokResponsesBodyWithClientToolsCompatibility(effective, upstreamModel, true)
	require.NoError(t, err)
	effective, _, err = normalizeGrokResponsesProtocolCompatibility(effective)
	require.NoError(t, err)
	want := resolveGrokCacheIdentity(c, effective, "", upstreamModel)
...
func TestResolveGrokWSCacheIdentitySchemaFallbackIsStableAndNotDoubleObserved(t *testing.T) {
	...
	prepared, err := prepareOpenAIWSHTTPBridgeBody(account, seed)
	require.NoError(t, err)
