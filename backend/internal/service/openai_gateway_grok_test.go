...
func TestSanitizeGrokResponsesToolsRemovesDeferredFlagsWithToolSearch(t *testing.T) {
	...
	patched, _, err := sanitizeGrokResponsesTools(body, false)
	require.NoError(t, err)
...
func TestForwardGrokResponsesInvalidEncryptedContentFallbackStillRetriesOnce(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	body := []byte(`{"model":"grok","input":[{"type":"reasoning","encrypted_content":""},{"type":"message","role":"user","content":"hi"}],"stream":false}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(body))
	...
	require.NotNil(t, result)
	require.Len(t, upstream.requests, 2)
	require.Equal(t, gjson.String, gjson.GetBytes(upstream.bodies[0], "input.0.encrypted_content").Type)
