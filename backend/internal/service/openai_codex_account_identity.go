...
const codexAccountIdentitySelectedContextKey = "openai_codex_account_identity_selected"
...
func codexAccountIdentitySource(c *gin.Context, fallback *Account) *Account {
	if c != nil {
		selected, _ := c.Get(codexAccountIdentitySelectedContextKey)
		if selectedAccount, ok := selected.(*Account); !ok || selectedAccount != fallback {
			return fallback
		}
