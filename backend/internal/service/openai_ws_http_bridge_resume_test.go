...
func TestOpenAIWSHTTPBridgeLaterTurn429RetriesCurrentTurnOnReplacementAccount(t *testing.T) {
	...
	require.Equal(t, upstream.requests[1].Header.Get("thread-id"), gjson.GetBytes(upstream.bodies[1], "client_metadata.thread_id").String())
	...
	require.Equal(t, upstream.requests[2].Header.Get("thread-id"), gjson.GetBytes(upstream.bodies[2], "client_metadata.thread_id").String())
	require.NotEqual(t, gjson.GetBytes(upstream.bodies[1], "client_metadata.thread_id").String(), gjson.GetBytes(upstream.bodies[2], "client_metadata.thread_id").String())
