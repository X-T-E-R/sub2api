export default {
    settings: {
      ...
      codexSessionAffinity: {
        ...
        behaviorEnrollment: '选中的分组会为新建的显式 Codex 根会话保存持久化的当前账号偏好。',
        behaviorCandidates: '仅每日会话池已启用的 OAuth/setup-token 候选凭据参与。并发繁忙时会在有界时间内等待；遇到真实配额、认证、账号禁用或不兼容失败时，可切换到其他合适账号并更新持久化偏好。每日会话分配规则不变。',
        behaviorRemoval: '移除分组后停止新会话加入；已有偏好仍可在符合条件的账号之间切换。缺少显式原始会话的请求继续使用旧路由。',
        groups: '可加入的分组',
        groupsHint: '只有 OpenAI 和 composite 分组可以加入新的 Codex 会话。已选但后来变为不适用的分组仍会保留在列表中，便于移除；移除不会清除已有会话偏好。',
        ineligible: '不适用于新加入',
        selectedCount: '已选择 {count} 个',
        noGroups: '暂无可用分组。',
        save: '保存偏好分组',
        saved: 'Codex 会话账号偏好已保存',
        loadFailed: '加载 Codex 会话账号偏好设置失败',
        saveFailed: '保存 Codex 会话账号偏好设置失败'
      },
      wechatConnect: {
        title: '微信登录',
      ...
      openaiExperimentalScheduler: {
        ...
        weightsTitle: '调度权值覆盖',
        weightsDescription: '留空时使用配置/环境变量值；配置未设置时使用内置默认值。页面非空设置优先。OpenAI 始终不将 TTFT 或历史错误率用于调度评分或账号逃逸，即使启用实验调度；这些指标仍适用于 Grok。',
        defaultPlaceholder: '配置/默认：{value}',
        topKLabel: 'TopK',
        priorityWeight: '优先级',
        loadWeight: '负载',
        queueWeight: '排队',
        errorRateWeight: '历史错误率（Grok）',
        ttftWeight: '首包延迟（Grok）',
        resetWeight: '重置窗口',
