...
const shouldShowResetTime = computed(() => {
  ...
  return Boolean(!props.remainingCapacity && props.showNowWhenIdle && props.utilization <= 0)
})

// Format reset time
const formatResetTime = computed(() => {
  // For rolling windows, when utilization is 0%, treat as immediately available.
  if (!props.remainingCapacity && props.showNowWhenIdle && props.utilization <= 0) {
  ...
  // 显示「待刷新」以区别于真正可用的「现在」。
  if (diffMs <= 0) {
    return props.remainingCapacity || props.utilization > 0 ? t('usage.resetPending') : t('usage.resetNow')
  }
